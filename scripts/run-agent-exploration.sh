...
BROWSER_CLIENT_HEADLESS="${BROWSER_CLIENT_HEADLESS:-true}"
BROWSER_CLIENT_BRIDGE_MODE="${BROWSER_CLIENT_BRIDGE_MODE:-controller}"
BROWSER_CLIENT_LABEL="${BROWSER_CLIENT_LABEL:-keepalive-controller}"
BROWSER_CLIENT_SESSION_ID="${BROWSER_CLIENT_SESSION_ID:-keepalive-${RUN_ID}}"
BROWSER_CLIENT_BRIDGE_PRIORITY="${BROWSER_CLIENT_BRIDGE_PRIORITY:-100}"
SNAPSHOT_BRIDGE_MODE="${SNAPSHOT_BRIDGE_MODE:-observer}"
SNAPSHOT_BRIDGE_LABEL_PREFIX="${SNAPSHOT_BRIDGE_LABEL_PREFIX:-snapshot}"
...
echo "[SETUP] controller_client mode=$BROWSER_CLIENT_BRIDGE_MODE label=$BROWSER_CLIENT_LABEL session=$BROWSER_CLIENT_SESSION_ID priority=$BROWSER_CLIENT_BRIDGE_PRIORITY"
if [ "$TAKE_SCREENSHOTS" = "1" ]; then
  echo "[SETUP] screenshots enabled -> $SHOTS_DIR (bridge_mode=$SNAPSHOT_BRIDGE_MODE)"
fi
...
VITE_AGENT_BRIDGE_WS_URL="$WS_URL" VITE_AGENT_BRIDGE_WS_PORT="$BRIDGE_PORT" npm run dev -- --host "$DEV_HOST" --port "$DEV_PORT" >"$DEV_LOG" 2>&1 &
DEV_PID=$!
...
if [ "$BROWSER_CLIENT_ENABLED" = "1" ]; then
  echo "[START] browser client -> $DEV_URL (headless=$BROWSER_CLIENT_HEADLESS bridge_mode=$BROWSER_CLIENT_BRIDGE_MODE label=$BROWSER_CLIENT_LABEL)"
  DEV_URL="$DEV_URL" BROWSER_CLIENT_HARD_REFRESH="${BROWSER_CLIENT_HARD_REFRESH:-1}" \
    PLAYWRIGHT_HEADLESS="$BROWSER_CLIENT_HEADLESS" \
    BROWSER_CLIENT_BRIDGE_MODE="$BROWSER_CLIENT_BRIDGE_MODE" \
    BROWSER_CLIENT_LABEL="$BROWSER_CLIENT_LABEL" \
    BROWSER_CLIENT_SESSION_ID="$BROWSER_CLIENT_SESSION_ID" \
    BROWSER_CLIENT_BRIDGE_PRIORITY="$BROWSER_CLIENT_BRIDGE_PRIORITY" \
...
if [ "$TAKE_SCREENSHOTS" = "1" ]; then
  ...
  DEV_URL="$DEV_URL" OUT_PATH="$SHOTS_DIR/00-before-agent.png" WAIT_MS="$SHOT_WAIT_MS" \
    SNAPSHOT_BRIDGE_MODE="$SNAPSHOT_BRIDGE_MODE" \
    SNAPSHOT_BRIDGE_LABEL="${SNAPSHOT_BRIDGE_LABEL_PREFIX}-before-${RUN_ID}" \
...
if [ "$TAKE_SCREENSHOTS" = "1" ]; then
  DEV_URL="$DEV_URL" OUT_PATH="$SHOTS_DIR/99-after-agent.png" WAIT_MS="1200" \
    SNAPSHOT_BRIDGE_MODE="$SNAPSHOT_BRIDGE_MODE" \
    SNAPSHOT_BRIDGE_LABEL="${SNAPSHOT_BRIDGE_LABEL_PREFIX}-after-${RUN_ID}" \
